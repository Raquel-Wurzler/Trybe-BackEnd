...
const BookServiceMock = require('./moks/book.service.mock');
...
describe('BookService', () => {
  describe('#getAll', () => {
    ...
    describe('quando existem livros cadastrados', () => {
      before(async () => {
        findAllStub.resolves([BookServiceMock.testBook]);
        books = await BookService.getAll();
      });
      ...
      it('o array deve retornar objetos', async () => {
        expect(books).to.be.deep.equal([BookServiceMock.testBook]);
      });
    });
  });
  ...
  describe('#getById', () => {
    ...
    describe('quando existe o livro', () => {
      before(async () => {
        findByPkStub.resolves(BookServiceMock.testBook);
      ...
      it('a resposta é um objeto contendo os dados do livro', async () => {
        expect(book).to.deep.equal(BookServiceMock.testBook);
      });
    });
